...
void a3ssfx_render(a3_DemoState const* demoState, a3_DemoMode2_SSFX const* demoMode, a3f64 const dt)
{
	...
	{
		// ****TO-DO:
		//X	-> uncomment deferred shading program and diffuse texture activations
		...
		//X		(hint: inverse bias-projection variable is commented out above)
		// draw light volumes
		currentDemoProgram = demoState->prog_drawPhongPointLight_instanced;
		a3shaderProgramActivate(currentDemoProgram->program);

		a3framebufferBindColorTexture(demoState->fbo_c16x4_d24s8, a3tex_unit04, 0); //texcood
		a3framebufferBindColorTexture(demoState->fbo_c16x4_d24s8, a3tex_unit05, 1); //normal
		a3framebufferBindColorTexture(demoState->fbo_c16x4_d24s8, a3tex_unit06, 3); //posiiton
	...
	case ssfx_renderModePhongDS:
		...
		a3framebufferBindColorTexture(demoState->fbo_c16x4_d24s8, a3tex_unit06, 3); //posiiton
		a3framebufferBindDepthTexture(demoState->fbo_c16x4_d24s8, a3tex_unit07); //depth
		a3framebufferActivate(writeFBO[ssfx_renderPassLights]);
		a3shaderUniformBufferActivate(demoState->ubo_light, demoProg_blockLight);
		a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uPB_inv, 1, projectionBiasMatInv.mm);
		//...
		break;
